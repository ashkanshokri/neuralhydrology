...
# Original pickle load function
original_pickle_load = pickle.load


# Define a custom caching version of pickle load function
@lru_cache(maxsize=None)
def cached_pickle_load(f):
    return original_pickle_load(f)


# Override pickle load function with the custom caching version
...
def _load_ensemble(results_files: List[Path], frequencies: List[str]) -> dict:
    """Averages the predictions of the passed runs and re-calculates metrics. """

    ensemble_results = defaultdict(dict)
    ensemble_results_path = defaultdict(dict)
    ...
    for run in tqdm(results_files):
        ...
        for basin, basin_results in run_results.items():
            for freq in frequencies:
                ...
                if freq not in ensemble_results[basin]:
                    ensemble_results[basin][freq] = [freq_results]
                    ensemble_results_path[basin][freq] = [str(run)]
                else:
                    #for target_var in target_vars:
                    ensemble_results[basin][freq].append(freq_results)
                    ensemble_results_path[basin][freq].append(str(run))

    for basin, basin_results in run_results.items():
        for freq in frequencies:
            ensemble_xr = xr.concat(ensemble_results[basin][freq], dim='member')
            ensemble[basin][freq] = ensemble_xr

    return ensemble


def _calculate_ensemble_metrics(ensemble_results,
                                frequencies,
                                target_vars,
                                clip_targets_to_zero=True,
                                config_metrics=None):
    ...
    for basin in tqdm(ensemble_results.keys()):
        for freq in frequencies:

            ensemble_xr = ensemble_results[basin][freq]
            ...
            for target_var in target_vars:
                ...
                if target_var in clip_targets_to_zero:
                    sim = xr.where(sim < 0, 0, sim)

                # calculate metrics
                metrics = config_metrics if isinstance(config_metrics, list) else config_metrics[target_var]
                if 'all' in metrics:
                    metrics = get_available_metrics()

                for member in list(range(len(ensemble_xr['member']))) + ['mean']:
                    try:
                        if member == 'mean':
                            member_obs = ensemble_xr[f'{target_var}_obs'].mean(dim='member')
                            member_sim = sim.mean(dim='member')
                        else:
                            member_obs = ensemble_xr[f'{target_var}_obs'].isel(member=member)
                            member_sim = sim.isel(member=member)

                        ensemble_metrics = calculate_metrics(member_obs, member_sim, metrics=metrics, resolution=freq)
                    ...
                    if len(ensemble_xr['member']) > 1:
                        ensemble_metrics = {f'{key}_mem{member}': val for key, val in ensemble_metrics.items()}
...
def _create_ensemble(results_files: List[Path], frequencies: List[str], config: Config) -> dict:
    ensemble_results = _load_ensemble(results_files, frequencies)
    return _calculate_ensemble_metrics(ensemble_results,
                                       frequencies,
                                       config.target_variables,
                                       clip_targets_to_zero=config.clip_targets_to_zero,
